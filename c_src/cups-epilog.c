...
generate_raster(FILE *pjl_file, FILE *bitmap_file)
{
    ...
    while (repeat--) {
        ...
        if (debug) {
            fprintf(stderr, "DEBUG: Raster orientation *r0F\n");
        }
        /* Raster power */
        ...
        if (debug) {
            fprintf(stderr, "DEBUG: Raster power &y%dP\n",
                (raster_mode == 'c' ||
                 raster_mode == 'g') ? 100 : raster_power);
        }
        /* Raster speed */
        ...
        if (debug) {
            fprintf(stderr, "DEBUG: Raster speed &z%dS\n", raster_speed);
        }
        fprintf(pjl_file, "\e*r%dT", height * y_repeat);
        if (debug) {
            fprintf(stderr, "DEBUG: Raster height *r%dT\n", height * y_repeat);
        }
        fprintf(pjl_file, "\e*r%dS", width * x_repeat);
        if (debug) {
            fprintf(stderr, "DEBUG: Raster width *r%dS\n", width * x_repeat);
        }
        /* Raster compression */
        fprintf(pjl_file, "\e*b%dM", (raster_mode == 'c' || raster_mode == 'g')
                ? 7 : 2);
        if (debug) {
            fprintf(stderr, "DEBUG: Raster compression *b%dM\n",
        ...
        if (debug) {
            fprintf(stderr, "DEBUG: Raster direction &y1O\n");
        }
        ...
        if (debug) {
            fprintf(stderr, "DEBUG: Start at current position *r1A\n");
        }
                ...
                for (pass = 0; pass < passes; pass++) {
                    ...
                    for (y = height - 1; y >= 0; y--) {
                        ...
                        if (l < h) {
                            ...
                            int r;
                            int n;
                            unsigned int pack_size = sizeof (buf) * 5 / 4 + 1;
                            unsigned char pack[pack_size];
                            ...
                            if (debug) {
                                fprintf(stderr, "DEBUG: Raster *b%dW\n", (n + 7) / 8 * 8);
                            }
                            r = 0;
                            while (r < n) {
